...
pub(crate) fn custom_error<T>(msg: &str, hresult: HRESULT) -> io::Result<T> {
    Err(io::Error::new(io::ErrorKind::Other, format!("{} (HRESULT = 0x{:08x})", msg, hresult)))
}
...
macro_rules! assert_hr {
    ($e:expr) => {
        ...
        if !winapi::shared::winerror::SUCCEEDED(hr) {
            panic!("HRESULT = 0x{:08x}", hr);
        }
    };
}

pub(crate) unsafe fn bstr(s: &str) -> winapi::shared::wtypes::BSTR {
    let mut s: Vec<_> = s.encode_utf16().collect();
    s.push(0);
    SysAllocString(s.as_ptr())
}
